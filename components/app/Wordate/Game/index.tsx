...
const Game: FC<IGameProps> = ({
  ...
  const [moveStack, setMoveStack] = useState<string[]>([firstWord]);
  ...
  const back = () => {
    setMoveStack((p) => {
      const newMoveStack = p.slice(0, -1);
      setCurrWord(newMoveStack.slice(-1)[0]);
      return newMoveStack;
    });
  };
    ...
    <div className="grid place-items-center align-baseline my-4">
      ...
      <div className="mt-4 p-1">
        <div className="flex">
          ...
          <Button
            disabled={disableUI || moveStack.length <= 1}
            type="red"
            onClick={() => back()}
            text="Back"
            svgEl={
